...
export function EnviarMensajeWhastapp(text, number) {
    ...
    // Palabras clave para volver atrás o reiniciar
    const palabrasVolver = ["volver", "atrás", "reiniciar", "menu"];

    if (palabrasVolver.includes(text)) {
        clientes[number].step = "solicitar_nombre"; // Reiniciar al paso inicial
        sendMessage(number, "👋 ¡Vamos a reiniciar! Por favor, dime tu *nombre* para continuar.");
        return;
    }
